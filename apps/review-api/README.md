...
The `POST /rag`, `POST /questionrag`, `POST /answerimagerag`, and
`POST /questionimagerag` endpoints accept the configured review outcomes:
`PRG`, `PRCR`, `PRCC`, `PRBD`, and `PRCS`. Retired outcomes are rejected. All
four endpoints also accept an empty `rag` string as an explicit reset, which
clears the existing review outcome cell after the normal UUID, sheet, and
content-state validation has succeeded. `PRNS` is the default state, not
another submitted trigger.
