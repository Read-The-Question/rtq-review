...
rag.answer.image.ignored = ["decorative"]
...
rag.question.image.ignored = ["decorative"]
...
test('parses the complete nested read model and safe preparation inputs', async () => {
  ...
  try {
    ...
    assert.deepEqual(question.review['answer-image'].imageIgnored, [
      'decorative',
    ]);
    assert.equal(
    ...
    assert.deepEqual(question.review['question-image'].imageIgnored, [
      'decorative',
    ]);
